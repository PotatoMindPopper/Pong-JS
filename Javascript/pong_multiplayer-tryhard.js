...
//In dit document staat de code om de normale singleplayer van Pong te spelen.
//De aanpassingen die zijn toegepast zijn het aanpassen van de variabelen. 
...
//AANPASSING: HET GAME OVER SCHERM IS TOEGEVOEGD OM ERVOOR TE ZORGEN DAT ER TEKST WORDT GETOOND, OM DE SPELER DUIDELIJK TE MAKEN WAT ER GEBEURD BIJ EEN SCORE VAN 10.	
	...
	if(speler2.score >= 10 || speler.score >= 10) {
		//om de score op het einde weer te geven
		tekenTekst('Het spel is afgelopen.', 15, 125, 'grey');
		tekenTekst('Score Speler1 is: '+speler.score, 15, 165, 'grey');
		tekenTekst('Score Speler2 is: '+speler2.score, 15, 205, 'grey');
...
cvs.addEventListener("keydown",beweegStok);
...
cvs.addEventListener("keydown",beweegStok2);

function beweegStok2(evt) {
	let rect = cvs.getBoundingClientRect();
	console.log("test key: "+evt.keyCode);
